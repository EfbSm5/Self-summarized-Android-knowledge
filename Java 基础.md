...
| 用法                         | 锁的范围   | 实际锁对象                           | 作用范围       |
| -------------------------- | ------ | ------------------------------- | ---------- |
| ① `synchronized(对象)`       | 代码块    | 括号里的对象                          | 仅该对象的同步块互斥 |
| ② `synchronized(this)`     | 当前实例对象 | this                            | 同一实例的同步块互斥 |
| ③ `synchronized(类名.class)` | 整个类    | 类的 Class 对象                     | 所有实例共享同一锁  |
| ④ `synchronized` 修饰方法      | 整个方法体  | 等价于锁 this（实例方法）或 类名.class（静态方法） | 整个方法互斥     |
...
# 线程上下文  
操作系统级别的执行上下文，包含线程栈、寄存器、调度信息以及线程局部变量等。线程上下文切换由操作系统调度器完成。
...
# 编译过程  
1. 源码编译，将.java编译成.class文件 
2. 类加载，有相应过程  
3. 执行，JIT实时执行字节码

# 泛型  
- 给类、接口、方法引入“类型参数”，让代码在编译期就进行类型检查与推断，避免强制类型转换，提升可读性与安全性。  
- 泛型擦除  
运行时类型被“擦除”为原始类型，T 在字节码层面多半变成 Object（配合边界转型）。  
- 通配符  
  1. 无界通配符：？
  2. 上界通配符：？extends X，为X 或其子类
  3. 下界通配符：？super X，为X 或其父类
...
# volatile
- 当一个线程修改了变量的值，会立即刷新到主内存；
其他线程读取时会强制从主内存重新读取最新值。
- 可以防止代码在编译或运行时被重新排序，从而保证执行顺序正确。
# CAS  
如果变量的内存值与期望值相同，则将变量更新为新，否则，说明有其他线程修改过，不做任何操作。
# ABA 
CAS的缺陷，CAS 只比较值是否相等，但不知道这个值在期间是否被改过再改回去。解决办法是加版本号或者加修改的布尔标记。

# 创建对象的过程  
1. 检查类是否已加载 → 类加载
2. 分配内存（堆）
3. 初始化为零
4. 设置对象头信息（Class 指针 + HashCode 等）
5. 调用构造函数 <init>
6. 返回引用地址

# 乐观锁悲观锁  
- 悲观锁（Pessimistic Lock）
先上锁、再操作，默认会发生冲突，所以在读/写之前就阻塞其他线程。
特点：强一致性、牺牲并发度、等待/阻塞成本高。
- 乐观锁（Optimistic Lock）
不加锁先操作，提交时检查是否有冲突（版本号/时间戳/期望值），冲突则重试。
特点：高并发友好、无阻塞、可能出现重试开销。

# jvm内存模型  （jvm运行时内存结构）
1. 程序计数器，存放当前线程所执行的字节码指令地址。
2. jvm栈，存放方法调用的局部变量表，方法返回地址等。
3. 本地方法栈，为Native设计的栈。
4. 堆，存放几乎所有对象的实例，是垃圾回收的主要区域。（所有线程共享）
5. 方法区，存放类的结构信息，静态变量。（所有线程共享）
6. 运行时常量池，存放类解析之后得到的各种字面量和符号引用。
...
## 异同
- 程序代码，在c存放在代码区，java存放在方法区
- 全局变量，c存放在全局区，java存放在方法区
- 局部变量，c和java都存放在栈
- 动态分配对象，c和java都存放在堆
- 字符串常量/常量，都存放在常量区
- 外部函数，系统调用，c存放在系统调用栈，java要用native stack